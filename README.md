...
## Служба заботы и админка (для `backend/`)

`POST /support` (из приложения, авторизованный запрос) сохраняет обращение
клиента в БД навсегда и, если настроена почта (см. `SMTP_*` выше),
отправляет письмо-уведомление на `SUPPORT_NOTIFICATION_EMAIL`.

Посмотреть все обращения можно в простой веб-админке:

```
http://localhost:3000/admin
```

Откроется окно логина браузера (HTTP Basic Auth) — логин/пароль задаются в
`backend/.env`:
- `ADMIN_USERNAME`
- `ADMIN_PASSWORD`

Без этих переменных `/admin` всегда отвечает 401 — паролей по умолчанию нет
специально. Страница — это один HTML-файл прямо в коде бэкенда
(`src/modules/admin/admin-page.html.ts`), без сборки и отдельного фронтенда;
данные подтягивает с `GET /admin/support-messages` (тот же логин).
